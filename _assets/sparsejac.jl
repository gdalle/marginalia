...
# ╔═╡ 5f1881fa-0568-4832-abc8-6e5e61850157
md"""
### DifferentiationInterface.jl for a unified pipeline
"""

# ╔═╡ cb4852e1-fc2c-4274-86f6-b27b7faec830
function sparsitydetect_modern(n)
	coloring_algorithm = SparseMatrixColorings.GreedyColoringAlgorithm()
	sparsity_detector = SparseConnectivityTracer.TracerSparsityDetector()
	backend = ADTypes.AutoSparse(
	...
	tdetect = @elapsed begin
		# performs sparsity detection and coloring
        DI.prepare_jacobian(A_h!, output, backend, input)
	end
	
	extras = DI.prepare_jacobian(A_h!, output, backend, input)
	jac = DI.jacobian(A_h!, output, backend, input, extras)
    tassemble = @elapsed begin
        DI.jacobian!(A_h!, output, jac, backend, input, extras)
    end
...
# ╔═╡ 62494da9-9f48-4a87-ba85-c56e3c0d143f
begin
	tx_modern=[ sparsitydetect_modern(n)[2:3] for n ∈ N]
	t_detect_modern=[ tx_modern[i][1] for i ∈ 1:length(N)]
	t_colorjac_modern=[ tx_modern[i][2] for i ∈ 1:length(N)]
end
...
let
	Plots.plot(xaxis=("N", :log10), yaxis=("time [s]", :log10), legend=:topleft)
    Plots.plot!(
		N, t_straight, label="ForwardDiff.jl",
		marker=(:auto,5), line=(:auto, 2))
    Plots.plot!(
		N, t_symbolic, label="Symbolics.jl",
		marker=(:auto,5), line=(:auto, 2))
    Plots.plot!(
		N, t_detect_modern, label="DI.jl (detection only)",
		marker=(:auto,5), line=(:auto, 2))
    Plots.plot!(
		N, t_colorjac_modern, label="DI.jl (differentiation only)",
		marker=(:auto,5), line=(:auto, 2))
    Plots.plot!(
		N, t_local, label="local Jacobians",
		marker=(:auto,5), line=(:auto, 2))
    Plots.plot!(
		N, t_vfvm, label="VoronoiFVM.jl",
		marker=(:auto,5), line=(:auto, 2))
    # Plots.loglog(N,1.2e-8*N.^2,"--",label=L"$O(n^2)$")
    # Plots.loglog(N,1.0e-6*N,"k--",label="O(n)")
    # Plots.loglog(N,0.9e-4*N,"k--")
...
manifest_format = "2.0"
project_hash = "00a8657b6bd12c2249955a9626da0e952e1d0118"
...
deps = ["AbstractTrees", "Bijections", "Dates", "DocStringExtensions", "ElasticArrays", "InteractiveUtils", "LinearAlgebra", "Printf", "Random", "SparseArrays", "StaticArrays", "StatsBase", "UUIDs", "WriteVTK"]
git-tree-sha1 = "96b3a26ccd52b40db56dc32fdb7f79579a167791"
...
git-tree-sha1 = "1157868b7595482f2cc2a6a38da641725022900e"
